...
export class ChurchConfigurationComponent extends BaseComponent implements OnInit {
  ...
  private all(route: string, size = 100): Observable<any> {const request = new RequestData(); request.offset = 0; request.size = size; return this.crud.onGetAll(route, request);}
  ...
  private syncAssignments(treasurers: any[], approvers: any[]): Observable<any> {
    ...
    users.forEach((user, id) => {const payload = {userConfiguration: user, treasurer: treasurerIds.has(id), financialApprover: approverIds.has(id)}; const current = existing.get(id); requests.push(current ? this.crud.onUpdate('churchResponsibleUser', current.id, payload) : this.crud.onSave('churchResponsibleUser', payload));});
  ...
  private showError(error: any): void {this.toast.error({summary: this.translate.translate('common_message'), detail: error.error?.message ?? this.translate.translate('common_message_error')});}
}
