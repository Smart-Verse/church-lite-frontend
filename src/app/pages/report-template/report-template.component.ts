...
export class ReportTemplateComponent implements OnInit{
 ...
 imageChanged(token:string){this.model.headerImage=token;}
 save(){this.saving=true;const request=this.model.id?this.http.put<ReportTemplate>('reportTemplate/'+this.model.id,this.model):this.http.post<ReportTemplate>('reportTemplate',this.model);request.subscribe({next:r=>{this.model=r;this.saving=false;this.toast.success({summary:this.translate.translate('common_message'),detail:this.translate.translate('report_template_saved')});},error:e=>this.error(e)});}
 private error(error:any){this.loading=this.saving=false;this.toast.error({summary:this.translate.translate('common_message'),detail:error?.error?.message??this.translate.translate('report_template_error')});}
}
