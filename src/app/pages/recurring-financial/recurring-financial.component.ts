...
export class RecurringFinancialComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.recurringId && this.recurringId !== 'new') this.load(this.recurringId);
      ...
      this.formGroup.patchValue({
        typeFinancial: this.types[1],
        recurrenceMode: this.modes[0],
  ...
  get installment(): boolean {
    return this.unwrap(this.formGroup.get('recurrenceMode')?.value) === 'INSTALLMENT';
  }
    ...
    request.subscribe({
      ...
      error: error => {
        ...
        this.toastService.error({
          summary: this.translateService.translate('common_message'),
          detail: error.error?.message ?? this.translateService.translate('common_message_error')
        });
      }
    });
  }
  ...
  private load(id: string): void {
    ...
    this.crudService.onGet('recurringFinancial', id).subscribe({
      next: data => {
        ...
        data.typeFinancial = this.find(this.types, data.typeFinancial);
        data.recurrenceMode = this.find(this.modes, data.recurrenceMode);
        data.frequency = this.find(this.frequencies, data.frequency);
        data.valueType = this.find(this.valueTypes, data.valueType);
        data.status = this.find(this.statuses, data.status);
  ...
  private setBreadcrumb(): void {
    this.breadcrumbItems = [{label: this.translateService.translate('financial_page_financial')}, {
      label: this.translateService.translate('financial_recurrence_title'),
      routerLink: ['/home/register/recurringFinancial']
    }, {label: this.translateService.translate(this.recurringId && this.recurringId !== 'new' ? 'financial_recurrence_edit' : 'financial_recurrence_new')}];
  }

  private navigateToList(): void {
    this.router.navigate(['/home/register/recurringFinancial']);
  }

  private options(group: string, values: string[]): any[] {
    return values.map(value => ({
      value,
      label: this.translateService.translate(`financial_recurrence_${group}_${value.toLowerCase()}`)
    }));
  }

  private unwrap(value: any): string {
    return value?.value ?? value;
  }

  private find(options: any[], value: string): any {
