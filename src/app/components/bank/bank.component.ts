...
export class BankComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.breadcrumbItems = this.makeBreadcrumb("entity_bank_title", "/home/register/bank");
    if (this.id && this.id !== "new") this.load(this.id);
  }

  private load(id: string): void {
    this.showLoading = true;
    this.crud.onGet("bank", id).subscribe({
      next: data => {
  ...
  onSave(): void {
    ...
    const request = this.id && this.id !== "new" ? this.crud.onUpdate("bank", this.id, dto) : this.crud.onSave("bank", dto);
    this.persist(request);
  }

  onCancel(): void {
    this.router.navigate(["/home/register/bank"]);
  }
  ...
  private makeBreadcrumb(title: string, link: string): MenuItem[] {
    return [{label: this.translateService.translate("entity_secretariat")}, {label: this.translateService.translate("financial_page_financial")}, {
      label: this.translateService.translate(title),
      routerLink: link
