...
export class PositionsComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.breadcrumbItems = [{label: this.translateService.translate("entity_secretariat")}, {label: this.translateService.translate("entity_others")}, {
      label: this.translateService.translate("entity_position_title"),
      routerLink: "/home/register/positions"
  ...
  private load(id: string): void {
    ...
    this.crud.onGet("positions", id).subscribe({
      next: data => {
        this.positionFormGroup.patchValue(data);
  ...
  onSave(): void {
    ...
    const dto = DTOConverter.convertPositionToDTO(this.positionFormGroup);
    const request = this.id && this.id !== "new" ? this.crud.onUpdate("positions", this.id, dto) : this.crud.onSave("positions", dto);
    this.persist(request);
  }

  onCancel(): void {
    this.router.navigate(["/home/register/positions"]);
  }
