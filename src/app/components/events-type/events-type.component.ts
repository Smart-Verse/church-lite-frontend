...
@Component({
  ...
  imports: [SharedCommonModule, ColorPickerModule, BreadcrumbModule],
...
export class EventsTypeComponent extends BaseComponent implements OnInit {
  ...
  private configuration = new EventTypesConfig();
  ...
  ngOnInit(): void {
    ...
    this.breadcrumbItems = [{label: this.translateService.translate("entity_secretariat")}, {label: this.translateService.translate("entity_others")}, {
      label: this.translateService.translate("entity_event_type_title"),
      routerLink: "/home/register/eventsType"
  ...
  private load(id: string): void {
    ...
    this.crud.onGet("eventsType", id).subscribe({
      next: data => {
  ...
  onSave(): void {
    ...
    const request = this.id && this.id !== "new" ? this.crud.onUpdate("eventsType", this.id, dto) : this.crud.onSave("eventsType", dto);
    this.persist(request);
  }

  onCancel(): void {
    this.router.navigate(["/home/register/eventsType"]);
  }
